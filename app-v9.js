(() => {
  const cfg = {
    key: 'cfly-rewards-we-in-here-2026-06-v9',
    max: 7,
  ...
  function freshState() {
    return { day: dayKey(), used: 0, done: false, busy: false, sound: true, speech: true, history: [] };
  }
  ...
  function start() {
    ...
    nodes.playButton.onclick = play;
    nodes.soundToggle.onclick = () => { state.sound = !state.sound; save(); draw(); say(state.sound ? 'Sound is on.' : 'Sound is off.'); };
    nodes.speechToggle.onclick = () => { state.speech = !state.speech; stopVoice(); stopPressPlayLoop(); if (!state.speech && 'speechSynthesis' in window) speechSynthesis.cancel(); save(); draw(); say(state.speech ? 'Game voice is on.' : 'Game voice is off. Screen reader text will still update.'); if (state.speech) startPressPlayLoop(1200); };
    draw();
  ...
  async function play(event) {
    ...
    if (state.done) { openPanel(); playClip('cflyTag', 'CFLY means Cazerny Forever Loves You.'); return; }
    ...
    try {
      ...
      if (ok) {
        ...
        await wait(1800);
        updateLive('CFLY means Cazerny Forever Loves You.');
        await playClipAndWait('cflyTag', 'CFLY means Cazerny Forever Loves You.', true, 2600);
      } else {
        ...
        if (left() === 1) {
          ...
          await playClipAndWait('lastPlay', 'You only have one more chance. This is your last play for today. Make it count.', true, 3200);
        }
      }
    } catch (e) {
  ...
  function playClipAndWait(name, fallbackText='', allowFallback=true, fallbackMs=2500) {
    ...
    return new Promise(resolve => {
      ...
      try {
        ...
        setTimeout(finish, Math.max(fallbackMs, 6200));
  ...
  function draw() { nodes.triesLeftDisplay.textContent = left(); nodes.downloadStatusDisplay.textContent = state.done ? 'Claim Ready' : 'Locked'; nodes.soundStatusDisplay.textContent = 'On'; nodes.playButton.disabled = state.busy || state.done || left() <= 0; nodes.playButton.textContent = state.busy ? 'Playing' : state.done ? 'Reward Ready' : left() <= 0 ? 'No Plays Left' : 'Play'; nodes.soundToggle.textContent = state.sound ? 'Turn Sound Off' : 'Turn Sound On'; nodes.speechToggle.textContent = state.speech ? 'Turn Game Voice Off' : 'Turn Game Voice On'; history(); if (state.done) openPanel(); }
  ...
  function load() { try { const raw = localStorage.getItem(cfg.key); const saved = raw && JSON.parse(raw); if (saved && saved.day === state.day) { state.used = Number(saved.used) || 0; state.done = !!saved.done; state.history = Array.isArray(saved.history) ? saved.history : []; } } catch(e) {} state.busy = false; state.sound = true; state.speech = true; }
  function save() { try { localStorage.setItem(cfg.key, JSON.stringify({ day: state.day, used: state.used, done: state.done, history: state.history })); } catch(e) {} }
