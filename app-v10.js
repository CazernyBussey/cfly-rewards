(() => {
  const cfg = {
    key: 'cfly-rewards-we-in-here-2026-06-v10',
    max: 7,
  ...
  function freshState() {
    return { day: dayKey(), used: 0, done: false, busy: false, started: false, sound: true, speech: true, history: [] };
  }
  ...
  function start() {
    ...
    draw();
    updateLive('Press Start to hear the intro.', false);
  }
  ...
  async function runIntro() {
    ...
    try {
      ...
      await wait(700);
      updateLive('Press play when you are ready.', true);
      await playClipAndWait('pressPlay', 'Press play when you are ready.', true, 2300);
      state.started = true;
      result('Ready. Press Play when you are ready.');
    } finally {
      state.busy = false;
      save();
      draw();
  ...
  function draw() { nodes.triesLeftDisplay.textContent = left(); nodes.downloadStatusDisplay.textContent = state.done ? 'Claim Ready' : 'Locked'; nodes.soundStatusDisplay.textContent = 'On'; nodes.playButton.disabled = state.busy || state.done || (state.started && left() <= 0); nodes.playButton.textContent = state.busy ? (state.started ? 'Playing' : 'Starting') : state.done ? 'Reward Ready' : !state.started ? 'Start' : left() <= 0 ? 'No Plays Left' : 'Play'; nodes.soundToggle.textContent = state.sound ? 'Turn Sound Off' : 'Turn Sound On'; nodes.speechToggle.textContent = state.speech ? 'Turn Game Voice Off' : 'Turn Game Voice On'; history(); if (state.done) openPanel(); }
  ...
  function load() { try { const raw = localStorage.getItem(cfg.key); const saved = raw && JSON.parse(raw); if (saved && saved.day === state.day) { state.used = Number(saved.used) || 0; state.done = !!saved.done; state.started = !!saved.started; state.history = Array.isArray(saved.history) ? saved.history : []; } } catch(e) {} state.busy = false; state.sound = true; state.speech = true; }
  function save() { try { localStorage.setItem(cfg.key, JSON.stringify({ day: state.day, used: state.used, done: state.done, started: state.started, history: state.history })); } catch(e) {} }
