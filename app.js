(() => {
  const cfg = {
    key: 'cfly-rewards-we-in-here-2026-06-v8',
    max: 7,
  ...
  function stopPressPlayLoop() {
    if (pressPlayTimer) clearTimeout(pressPlayTimer);
    pressPlayTimer = null;
  }
  ...
  function playClipAndWait(name, fallbackText='', allowFallback=true, fallbackMs=2500) {
    ...
    return new Promise(resolve => {
      ...
      try {
        ...
        if (allowFallback) fallbackSpeech(fallbackText);
  ...
  function playClip(name, fallbackText='', allowFallback=true, onEnded=null) {
    if (!state.speech && name !== 'cflyTag') return;
    const src = cfg.voice[name];
  ...
  function build(force) {
    if (force) { const one = pick(cfg.items.filter(x => x[3])); return [one, one, one]; }
    return [pick(cfg.items), pick(cfg.items), pick(cfg.items)];
  }
  ...
  function draw() {
    ...
    nodes.soundStatusDisplay.textContent = state.sound ? 'On' : 'Off';
    nodes.playButton.disabled = state.busy || state.done || left() <= 0;
    nodes.playButton.textContent = state.busy ? 'Playing' : state.done ? 'Reward Ready' : left() <= 0 ? 'No Plays Left' : 'Play';
