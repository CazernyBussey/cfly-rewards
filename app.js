(() => {
  const cfg = {
    key: 'cfly-rewards-we-in-here-2026-06',
    max: 7,
    delay: 700,
    ...
    voice: {
      ...
      tileOne: './assets/voice/04-tile-one.mp3',
      tileTwo: './assets/voice/05-tile-two.mp3',
      tileThree: './assets/voice/06-tile-three.mp3',
  ...
  async function play() {
    ...
    state.used += 1;
    save(); draw();
    if (left() === 0) say('This is your last play for today. Make it count.', true, false, 'lastPlay');
    moving(true);
    ...
    for (let i = 0; i < 3; i++) { setTile(i, picked[i]); tone('tick'); announceTile(i, picked[i]); await wait(320); }
    moving(false);
    ...
    if (ok) {
      state.done = true;
      result(`Tile one is ${picked[0][1]}, tile two is ${picked[1][1]}, tile three is ${picked[2][1]}. CFLY! You matched three. Enter your email to claim your reward.`);
      tone('high');
      openPanel();
      say('CFLY! You matched three. Enter your email to claim your reward.', true, true, 'rewardUnlocked');
      setTimeout(() => { if (state.speech) playClip('cflyTag'); }, 1500);
    } else {
      result(`${picked.map((x,i)=>`tile ${i+1} is ${x[1]}`).join(', ')}. Not this time. Run it back. You have ${left()} ${left() === 1 ? 'play' : 'plays'} left today.`);
      tone('low');
  ...
  function announceTile(i, item) {
    const words = ['Tile one', 'Tile two', 'Tile three'];
    const clips = ['tileOne', 'tileTwo', 'tileThree'];
    say(`${words[i]}: ${item[1]}.`, true, false, clips[i]);
  }
  ...
  function say(text, voice=false, urgent=false, clip=null) {
    ...
    if ('speechSynthesis' in window) { speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(text); u.rate=.94; speechSynthesis.speak(u); }
  }
