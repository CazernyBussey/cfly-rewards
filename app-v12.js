(() => {
  const cfg = {
    key: 'cfly-rewards-we-in-here-2026-06-v12',
    max: 7,
  ...
  function history() {
    nodes.playHistory.innerHTML = '';
    if (!state.history.length) {
      const li = document.createElement('li');
      li.textContent = 'No plays yet.';
      nodes.playHistory.appendChild(li);
      return;
    }
    state.history.forEach(h => {
      const li = document.createElement('li');
      li.textContent = `${h.ok ? 'Matched' : 'No match'}: ${h.labels.join(', ')}.`;
      nodes.playHistory.appendChild(li);
    });
  }
  ...
  function tone(kind) {
    ...
    try {
      ...
      const map = { start:[180,.16,.07], tick:[520,.08,.06], high:[880,.42,.09], low:[170,.18,.05] };
